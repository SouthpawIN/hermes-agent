...
def _force_rmtree(path: Path) -> None:
    """Recursively remove *path*, tolerating read-only files / dirs.

    Some distribution-owned trees (e.g. files copied out of a git clone, or
    materialised by another profile) can contain read-only entries that cause
    ``shutil.rmtree`` to raise ``PermissionError``. This helper retries with
    an ``onerror`` handler that rewrites the permissions (on both the target
    and its parent — the parent's write bit is what governs removal) and
    retries the operation, and falls back to ``sudo rm -rf`` as a last resort.
    """
    def _on_error(func, target, exc_info):
        # Removal is governed by the *parent* directory's write bit, but the
        # target itself may also be read-only (e.g. a file that can't be
        # unlinked on some platforms). Fix up both, then retry; if it still
        # fails, let the exception propagate so the sudo fallback runs.
